...
public class RelatoController {
    ...
    public ResponseEntity<List<RelatoResponse>> getAll() {
        List<RelatoResponse> relatos = this.service.getAll();
        if (relatos.isEmpty()) {
    ...
    public ResponseEntity<List<RelatoResponse>> getByVisitaId(@PathVariable Long visitaId) {
        List<RelatoResponse> relatos = this.service.getAllByVisitaId(visitaId);
        if (relatos.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(relatos);
    ...
    @Operation(summary = "Get Relatos by Base ID", description = "Retrieve all relatos associated with a specific base ID.")
    public ResponseEntity<List<RelatoResponse>> getByBaseId(@PathVariable Long baseId) {
        List<RelatoResponse> relatos = this.service.getAllByBaseId(baseId);
        if (relatos.isEmpty()) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(relatos);
    ...
    public ResponseEntity<RelatoResponse> create(@RequestBody @Valid RelatoRequest request) {
        ...
        RelatoResponse response = this.service.createRelato(request);
    ...
    public ResponseEntity<RelatoResponse> update(@PathVariable Long id, @RequestBody @Valid RelatoRequest request) {
        ...
        RelatoResponse response = this.service.updateRelato(id, request);
        return ResponseEntity.ok(response);
    }
