...
import { Alert, Box, CircularProgress, IconButton, Tab, Tabs, Typography, useMediaQuery, useTheme } from "@mui/material";
...
function TabPanel({ children, value, index, isMobile, ...other }: TabPanelProps) {
    return (
        <div
            role="tabpanel"
            hidden={value !== index}
        ...
        >
            {value === index && (
                <Box sx={{ p: isMobile ? 1 : 3 }}>{children}</Box> // 👈 padding reduzido no mobile
            )}
        </div>
    );
}
...
export default function VisitaDetailPage() {
    ...
    if (!visita) return <Alert severity="warning">Visita não encontrada</Alert>;
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDateFns}>
            ...
            <Box sx={{ p: isMobile ? 1 : 3 }}>
                ...
                {/* Cabeçalho — título menor no mobile */}
                <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                    <IconButton
                        onClick={() => router.push(`/admin/bases/${baseId}/visitas`)}
                        sx={{ mr: 1 }}
                    >
                        <BackIcon />
                    </IconButton>
                ...
                {/* Tabs com scroll no mobile para não cortar os labels */}
                <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                    <Tabs
                        value={tabValue}
                        onChange={handleTabChange}
                        aria-label="abas de detalhes da visita"
                        variant={isMobile ? "scrollable" : "standard"} // 👈 scroll horizontal no mobile
                ...
                <TabPanel value={tabValue} index={2} isMobile={isMobile}>
                    <ChecklistsTab visitaId={visitaId} />
                </TabPanel>
            </Box>
