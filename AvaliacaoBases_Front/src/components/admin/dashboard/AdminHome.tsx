...
import { ChartCard, InfoSection, Placeholder } from './utils';
...
export default function AdminHomePage({ baseId }: { baseId?: string }) {
    ...
    const VisitMetric = ({ icon, title, value, color }: { icon: React.ReactNode; title: string; value: string | number; color: string }) => (
        <Box
            sx={{
                display: 'flex',
                alignItems: 'center',
                ...
                <Box sx={{ maxWidth: 1480, mx: 'auto' }}>
                    <Box sx={{ mb: { xs: 2.5, md: 3.5 } }}>
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.75, maxWidth: 760 }}>
                            Acompanhe visitas, indicadores operacionais, padronização visual e conformidade das bases no período selecionado.
                        </Typography>
                    </Box>
                        ...
                        <>
                            ...
                            {resumo && !loading && (
                                ...
                                <Paper variant="outlined" sx={sectionPaperSx}>
                                    ...
                                    {/* ══ TABELAS ═══════════════════════════════════════ */}
                                    <Box sx={{ mb: 4 }}>
                                        <SectionHeader title="Indicadores operacionais" caption="Médias calculadas a partir dos últimos envios disponíveis." />
                                        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 3 }}>
                                            {[
                                                {
                                                    title: 'Tempo de Resposta e Prontidão por Município',
                                                    rows: dadosFiltrados,
                                                    cols: [
                                                ...
                                                {
                                                    title: 'Atividade das Viaturas por Município',
                                                    rows: vtrFiltradas,
                                                    cols: [
                                                        { header: 'Município', key: 'cidade', align: 'left' as const },
                                                        ...
                                                        <Table size="small" sx={{ minWidth: 260, borderCollapse: 'separate', borderSpacing: 0 }}>
                                                            ...
                                                            <TableBody>
                                                                {(rows as any[]).map((row, ri) => (
                                                                    <TableRow key={ri} sx={{ bgcolor: 'background.paper', '&:hover': { bgcolor: 'rgba(25, 118, 210, 0.04)' } }}>
                                                                        {cols.map(c => (
                                    ...
                                    {/* ══ GRÁFICOS PRINCIPAIS ══ */}
                                    <SectionHeader title="Avaliações e conformidade" caption="Comparativo visual das bases, categorias e status operacional." />
                                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 1fr' }, gap: 3, mb: 4, alignItems: 'flex-start', width: '100%' }}>

                                        {/* ─── coluna esquerda ─── */}
                                        <Box sx={{ width: '100%', minWidth: 0, gap: 2, display: 'flex', flexDirection: 'column' }}>
