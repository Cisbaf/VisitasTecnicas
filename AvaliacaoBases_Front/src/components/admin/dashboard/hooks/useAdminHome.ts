...
export function useAdminHome() {
        ...
        try {
            ...
            if (!selectedMunicipio) {
                try {
                    ...
                    console.log('todasVisitas:', todasVisitas);
                ...
                for (const base of basesToProcess) {
                    try {
                        ...
                        console.log("Visitas para base específica:", base.id, visitasData);
                        const visitas = Array.isArray(visitasData) ? visitasData.filter(v => v.tipoVisita === null || v.tipoVisita === "Inspeção" || v.tipoVisita === '') : [];
                        console.log('visitas filtradas para base específica filtrada:', base.id, visitas);
    ...
    const processarBase = async (
        ...
        // ======================= INSPEÇÃO =======================
        // 1. Ordenar visitas da base (mais recente primeiro)
        ...
        // 2. Encontrar a visita mais recente que tenha formulários de inspeção
        let ultimaVisita = null;
        for (const visita of visitasOrdenadas) {
            const formsInspecaoDaVisita = inspectionForms.filter((form: any) =>
                String(form.visitaId) === String(visita.id)
            );
            if (formsInspecaoDaVisita.length > 0) {
                ultimaVisita = visita;
                break;
            }
        }

        // Se não houver visita de inspeção, define avg como 0 e pula o processamento
        if (!ultimaVisita) {
