...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return await proxyFetch(`/avaliacao/user/${encodeURIComponent(id)}`, {
            headers: { Authorization: `Bearer ${token}` },
            cache: "no-store",
        });
    } catch (err) {
        console.error("api/checklist/[id] GET proxy error:", err);
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        console.error("api/checklist/[id] DELETE proxy error:", err);
        return NextResponse.json({ message: "Erro interno", detail: String(err) }, { status: 500 });
    }
}