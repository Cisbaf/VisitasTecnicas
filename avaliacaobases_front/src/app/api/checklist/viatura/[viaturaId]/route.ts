...
export async function GET(req: Request, { params }: { params: Promise<{ viaturaId: string }> }) {
    try {
        const { viaturaId } = await params;
        const cookieStore = await cookies();
        const token = cookieStore.get("token")?.value;
        if (!token) return NextResponse.json({ message: "Unauthorized" }, { status: 401 });

        return await proxyFetch(`/checklist/viatura/${encodeURIComponent(viaturaId)}`, {
            headers: { Authorization: `Bearer ${token}` },
            cache: "no-store",
        });
    } catch (err) {
        console.error("api/checklist/visita/[visitaId] GET proxy error:", err);
